...
export function IconSidebar({ isExpanded, onToggle, datasetName, onAddSheet, onShare, sheets, activeSheetId, onSheetSelect, onSheetRename }: IconSidebarProps) {
  ...
  return (
    ...
    >
      ...
      {/* Sheets List */}
      {isExpanded && sheets && sheets.length > 0 && (
        <div className="flex-1 overflow-y-auto border-t border-primary-foreground/20">
          <div className="p-2">
            <h3 className="px-4 py-2 text-xs font-semibold text-primary-foreground/70 uppercase">Sheets</h3>
            <div className="space-y-1">
              {sheets.map((sheet) => (
                <div
                  key={sheet.id}
                  className={`group flex items-center justify-between px-4 py-2 rounded cursor-pointer transition-colors ${
                ...
                >
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      const newName = window.prompt('Rename sheet', sheet.name)
                      if (newName && newName !== sheet.name) {
                        onSheetRename?.(sheet.id, newName)
                      }
                    }}
                    className="opacity-0 group-hover:opacity-100 p-1 hover:bg-primary-foreground/20 rounded transition-opacity"
                    title="Rename sheet"
                  >
